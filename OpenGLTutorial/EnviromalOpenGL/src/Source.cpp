...
int main()
{
	...
	//*                                                                                                              *//
	GLuint VBO, containerVAO;						// VBO - Vertex Buffer Object, VAO - Vertex Array Object, EBO - Element Buffer Object
	glGenVertexArrays(1, &containerVAO);			// Generate VAO
	...
	// We only need to bind to the VBO (to link it eith glVertexAttribPointer), so need to fill it, the VBO's data already contains all we need.
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	...
	unsigned char* image;
	// Diffuse map
	image = SOIL_load_image("packSource/container2.png", &widthImg, &heightImg, 0, SOIL_LOAD_RGB);
	...
	// Specular Map
	image = SOIL_load_image("packSource/container2_specular.png", &widthImg, &heightImg, 0, SOIL_LOAD_RGB);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Use cooresponding shader when setting uniform/drawing objects
		lightingShader.Use();
		...
		// Also draw the lamp object, again binding the appropirate shader
		lampShader.Use();
